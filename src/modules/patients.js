...
const savePatients = () => {
  ...
  fs.writeFileSync("src/patients.json", JSON.stringify(patients, null, 2))
}
export const loadPatients = () => {
  if (process.env.NOFS !== undefined) {
    return
  }
  patients = JSON.parse(fs.readFileSync("src/patients.json"))
}

export const getNumPatients = (docId) => {
  return patients.filter((patient) => docId === patient.doctorId).length
}
